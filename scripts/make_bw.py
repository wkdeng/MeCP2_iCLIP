...
def parse_chunk_gene(args):
    ...
    unique_only = args[3]
    scale_factor=args[4]
    unstranded=args[-1] if len(args)==6 else False
...
def parse_one_sample(bam_list, chr_size, annotation, output_folder, sample, threadN, unique_only,unstranded=False):
    ...
    list_list = pool.map(parse_chunk_gene, [
        (bam_list, gene_anno, child_gene_list[i], unique_only,scale_factor,unstranded) for i in range(threadN)])
    pool.terminate()
    pool.join()
...
def parse():
    ...
    parse_one_sample([options.realigned_bam, options.unique_bam],
                     ...
                     options.output_folder, options.sample_name,  options.threadN, options.unique_only, options.unstranded)
